...
The security issues are all shown in a simple Visual Studio solution. This solution includes fully commented code samples that show how to avoid each security breach. You will need to have DevExpress ASP.NET MVC extensions installed to load and compile the solution. You can download the installer from the [DevExpress website](https://devexpress.com).
...
This section describes how to handle file uploads securely. There are several separate scenarios to cover:
...
The table below lists the default file extensions allowed by various controls that support file uploads:
...
- The Spreadsheet and Rich Edit do not set a maximum size for an uploaded file, however there is a 31,457,280 byte limit for images to be inserted into a document. Note that both controls must be explicitly configured to accept file uploads.
...
Other operations on files organized by the File Manager (copy, delete, download, etc.) are configured through the [`SettingsEditing`](http://help.devexpress.com/#AspNet/DevExpressWebMvcFileManagerSettings_SettingsEditingtopic) property. All such operations are disabled by default.
...
To avoid a security breach, you need to ensure that these files are inaccessible to third parties.
...
2. Use the [Binary Image](http://help.devexpress.com/#AspNet/CustomDocument9010) extension to upload images. This control automatically implements an image file type check.
...
2. Make sure to specify the maximum uploaded file size to prevent those DoS attacks based on large file uploads.
...
In ASP.NET MVC, you can use the `[Authorize]` attribute to define authorization rules for a web application's pages on the controller side. This attribute can be applied either to a controller action or to the whole controller class.

For maximum security, it is recommended that you apply the `[Authorize]` attribute to the controller and use the `[AllowAnonymous]` attribute to allow anonymous access to actions that should be public.
...
To prevent errors in the browser when unauthorized access attempts are handled, check the access rights on the page's [`PageLoad`](https://github.com/DevExpress/aspnet-security-bestpractices/blob/408c2328fc8d567281994b2bba52d0705850c0b5/SecurityBestPractices/Authorization/Reports/ReportDesignerPage.aspx.cs#L6-L13) event. If the user is not authorized to open the report, redirect them to a public page.
...
After you implement your custom report storage with the required access rules, you must register it in the [Global.asax.cs](https://github.com/DevExpress/aspnet-security-bestpractices/blob/master/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Global.asax.cs) file:
...
> Note that to simplify the example project, the logger implementation obtains the required user account data from a static property. This is not a recommended solution: such an implementation could have difficulties running in a cloud environment or on a web farm. Instead, we recommend that you store authentication information in some appropriate data storage.

To familiarize yourself with the solution:

1. Run the example application and log in from the [Login](https://github.com/DevExpress/aspnet-security-bestpractices/blob/77534a317c3d685810e57cf034994d99a2d1c58f/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Controllers/AccountController.cs#L21-L24) page.
...
The [Report Designer](https://documentation.devexpress.com/XtraReports/17103/Creating-End-User-Reporting-Applications/Web-Reporting/Report-Designer) allows a user to browse available data connection and data tables in the integrated [Query Builder](http://help.devexpress.com/#AspNet/CustomDocument120076) UI. Refer to the [Query Builder](#33-query-builder) subsection in this document to learn how to restrict access to this information based on authorization rules.

### 3.3. Dashboard

The [DevExpress Web Dashboard](https://documentation.devexpress.com/Dashboard/16977/Building-the-Designer-and-Viewer-Applications/Web-Dashboard/ASP-NET-MVC-Dashboard-Extension) requires a controller, that inherits the **DashboardController** class. This class exposes predefined actions that handle all AJAX requests from the dashboard.
...
With this custom implementation of a dashboard storage, if a user named 'John' tries to use the client API to open a report with restricted access (e.g., a report with id='1'), the handler returns `error 404, File Not Found`:
...
The Web Dashboard control allows a user to browse available data connection and data tables in the integrated [Query Builder](http://help.devexpress.com/#AspNet/CustomDocument120076) UI. Refer to the [Query Builder](#33-query-builder) subsection in this document to learn how to restrict access to this information based on authorization rules.

### 3.4. Query Builder

The standalone [Query Builder](http://help.devexpress.com/#AspNet/CustomDocument120076) as well as the Query Builder integrated into the Report and Dashboard designers allows an end-user to browse a web application's data connections and the data tables available through those connections. In a web application that uses access control, you need to implement custom logic that restricts a user's access to the available connections and data tables.
...
1. Generate an anti-forgery token for a form in the [view code](https://github.com/DevExpress/aspnet-security-bestpractices/blob/master/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Views/UsingAntiForgeryToken/EditForm.cshtml#L21) as shown below:
   ...
   />
   ```
2. Be aware that complex AJAX-enabled controls do not automatically include the token field's value in their AJAX requests. You need to manually add the token value to such requests as shown in the code sample below:
...
> Note that you should never use the **GET** method to perform data modifying requests.
...
The following image demonstrates an error message that exposes sensitive information:
...
In the secure configuration, the error message is substituted by a more generic one:
...
A general recommendation is to always perform separate queries for data sources whose data is displayed in UI. These queries should never request data that should be kept in secret.
...
The DevExpress default HTTP handler (DXR.axd) serves static files including images, scripts and styles. We assume that these static files are intended for public access and do not expose any sensitive information or server-side code. However, there are additional recommendations on custom scripts and styles:
...
It is strongly suggested that you always sanitize page contents that can be specified by a user. You should be aware of what kind of sanitization you use so it is both compatible with the displayed content and guarantees the intended level of safety. For example, you can remove HTML tags throughout the content but it can corrupt text that is intended to contain code samples. A more generic approach would be to substitute all '<', '>' and '&' symbols with `&lt;`, `&gt;` and `&amp;` character codes.
...
Before you insert user input into a JavaScript block, you need to use the `HttpUtility.JavaScriptStringEncode` method to prepare the string and then call `@Html.Raw` to insert this string into the script:
...
To get familiar with the vulnerability, open the example project's [EncodeHtmlInControlsPartial.cshtm](https://github.com/DevExpress/aspnet-security-bestpractices/blob/master/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Views/HtmlEncoding/EncodeHtmlInControlsPartial.cshtml) view and uncomment the [following line](https://github.com/DevExpress/aspnet-security-bestpractices/blob/master/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Views/HtmlEncoding/EncodeHtmlInControlsPartial.cshtml#L14):
...
On form submit, the corresponding [controller action](https://github.com/DevExpress/aspnet-security-bestpractices/blob/master/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Controllers/UserInputValidation/UserInputValidation.cs#L20) will receive the model-bound input's value. Here you can use the `ModelState.IsValid` property to check for the user input's validity:
...
In many cases, your input forms do not display editors for all properties of the model because some properties are not intended to be edited by a user. For example, the model contains a `salary` field that should never be modifiable by an end-user.
...
You can use the `Bind` attribute's `Exclude` property as [shown below](https://github.com/DevExpress/aspnet-security-bestpractices/blob/master/SecurityBestPractices.Mvc/SecurityBestPractices.Mvc/Models/UserInputValidationModel.cs#L10) to prohibit model binding for a specific model property:
...
In addition to data annotation attributes supported by ASP.NET MVC, DevExpress ASP.NET MVC extensions support supplementary attributes that can be used for validation both on the server and client:
...
The `DateRange` attribute ties two `DateTime` properties together so that you to check whether a value falls within a specific date range.
...
In some cases, you may need to validate values of inputs that are not bound to a model. For example, you may want to validate a password that a user types as a plain string but that is stored as hash. A user types the password into an editor without model binding:
...
On server you can access this editor's value through a controller action's parameter with the same name as the input. You can use arbitrary logic to validate this value. If validation did not pass, you can use the `ModelState.AddModelError` method to register a validation error. This method automatically sets the `IsValid` property to `false`.
...
A special case of custom validation is validation of list items. Consider a use-case scenario where an online shop application allows a user to select a gift option from a list based on the state of their bonus account (see the example project's **UserInputValidation/ListEditors**) page.
...
3. Use this page to submit a forged request with the `Id=3`, which should not be available to an end user.
...
ASP.NET checks input values for potentially dangerous content. For example, if an end-user types `<b>` into a text input and submits the form, they are redirected to an error page with the following message:
...
Because of these reasons, the inbuilt request value checks are commonly disabled. Use the `ValidateInputAttribute` to do this:
...
This setting is not enabled by default because escaped content (everything that starts with "=", "-", "+", "@", "") in CSV can be unacceptable in many use-case scenarios. For example this can brake text values that start with "=" or negative numbers. For example:
...
> Note that an attacker can send POST requests to call controller methods without a View, as demonstrated in the example application:
...
Consider a use-case scenario when a web application receives a URL from an end-user, downloads an image file from this URL and saves the file in a database. The Binary Image extension is then used to display this file on the application's page.
...
In this case, an attempt to download a local file generates an exception.
